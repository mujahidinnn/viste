import {
  EditOutlined,
  GlobalOutlined,
  ReadOutlined,
  ShoppingOutlined,
  StarOutlined,
  UserOutlined,
} from "@ant-design/icons";
import { formatPeriod } from "../utils/formatPeriod";

export default function CVPreview4({ data }) {
  const {
    name,
    position,
    email,
    phone,
    address,
    summary,
    experiences = [],
    education = [],
    skills = [],
    languages = [],
    avatarUrl,
    rounded = 50,
    useIcon = true,
    useHeader = true,
    colorTheme = {
      text1: "#222222",
      text2: "#555555",
      text3: "#777777",
      bg: "#F0F0F0",
    },
  } = data || {};

  const { text1, text2, text3, bg } = colorTheme || {};

  return (
    <div
      style={{
        fontFamily: "'Inter', Arial, sans-serif",
        background: "#ffffff",
        color: text1,
        lineHeight: 1.6,
        overflow: "hidden",
      }}
    >
      {/* ===== HEADER ===== */}
      {useHeader && (
        <div
          style={{
            background: bg,
            display: "flex",
            alignItems: "center",
            padding: "24px 24px",
            borderBottom: `2px solid ${text3}`,
          }}
        >
          <img
            src={avatarUrl || "https://placehold.co/100x100"}
            alt="avatar"
            style={{
              width: 100,
              height: 100,
              borderRadius: `${rounded}%`,
              objectFit: "cover",
              marginRight: 24,
              border: `3px solid #fff`,
            }}
          />

          <div>
            <h1
              style={{
                margin: 0,
                fontSize: 26,
                fontWeight: 700,
                color: text1,
              }}
            >
              {name || "Nama Lengkap"}
            </h1>
            {position && (
              <h3
                style={{
                  margin: "4px 0 0 0",
                  fontWeight: 500,
                  color: text2,
                }}
              >
                {position}
              </h3>
            )}
          </div>
        </div>
      )}

      {/* ===== BODY ===== */}
      <div style={{ padding: "24px 0 24px 0" }}>
        <SectionHeading
          title="Data Pribadi"
          icon={useIcon ? <UserOutlined /> : null}
          colorTheme={bg}
          textColor={text1}
          borderColor={text3}
        />
        <InfoRow label="Nama" value={name || "Nama Lengkap"} color={text2} />
        <InfoRow
          label="Telepon"
          value={phone || "08xx-xxxx-xxxx"}
          color={text2}
        />
        <InfoRow
          label="Email"
          value={email || "email@contoh.com"}
          color={text2}
        />
        <InfoRow label="Alamat" value={address || "-"} color={text2} />

        {summary && (
          <>
            <SectionHeading
              title="Ringkasan Profil"
              icon={useIcon ? <EditOutlined /> : null}
              colorTheme={bg}
              textColor={text1}
              borderColor={text3}
            />
            <div
              style={{
                textAlign: "justify",
                color: text2,
                fontSize: 14,
                borderLeft: `4px solid ${bg}`,
                paddingLeft: 12,
                marginBottom: 24,
              }}
              dangerouslySetInnerHTML={{ __html: summary }}
            />
          </>
        )}

        {skills.length > 0 && (
          <>
            <SectionHeading
              title="Keterampilan"
              icon={useIcon ? <StarOutlined /> : null}
              colorTheme={bg}
              textColor={text1}
              borderColor={text3}
            />
            <ul style={{ marginTop: 8, color: text2 }}>
              {skills.map((s, i) => (
                <li key={i}>{s?.skill}</li>
              ))}
            </ul>
          </>
        )}

        {languages.length > 0 && (
          <>
            <SectionHeading
              title="Bahasa"
              icon={useIcon ? <GlobalOutlined /> : null}
              colorTheme={bg}
              textColor={text1}
              borderColor={text3}
            />
            <ul style={{ marginTop: 8, color: text2 }}>
              {languages.map((lang, i) => (
                <li key={i}>
                  {lang?.language}{" "}
                  {lang?.level ? (
                    <span style={{ color: text3 }}>– {lang.level}</span>
                  ) : null}
                </li>
              ))}
            </ul>
          </>
        )}

        {experiences.length > 0 && (
          <>
            <SectionHeading
              title="Pengalaman Kerja"
              icon={useIcon ? <ShoppingOutlined /> : null}
              colorTheme={bg}
              textColor={text1}
              borderColor={text3}
            />
            {experiences.map((exp, i) => {
              const e = exp || {};
              return (
                <div
                  key={i}
                  style={{
                    marginBottom: 20,
                    borderBottom: "1px solid #eee",
                    paddingBottom: 10,
                    color: text2,
                  }}
                >
                  <strong style={{ color: text1 }}>
                    {e.position || "Jabatan"}
                  </strong>
                  <div style={{ fontStyle: "italic", color: text2 }}>
                    {e.company || "Perusahaan"}
                  </div>
                  <div style={{ color: text3, fontSize: 13 }}>
                    {formatPeriod(e)}
                  </div>
                  {e.description && (
                    <div
                      style={{ marginTop: 6 }}
                      dangerouslySetInnerHTML={{
                        __html: e.description,
                      }}
                    />
                  )}
                </div>
              );
            })}
          </>
        )}

        {education.length > 0 && (
          <>
            <SectionHeading
              title="Pendidikan"
              icon={useIcon ? <ReadOutlined /> : null}
              colorTheme={bg}
              textColor={text1}
              borderColor={text3}
            />
            {education.map((edu, i) => {
              const e = edu || {};
              return (
                <div
                  key={i}
                  style={{
                    marginBottom: 20,
                    borderBottom: "1px solid #eee",
                    paddingBottom: 10,
                    color: text2,
                  }}
                >
                  <strong style={{ color: text1 }}>
                    {e.major || "Jurusan"}
                  </strong>
                  <div style={{ fontStyle: "italic", color: text2 }}>
                    {e.school || "Institusi"}
                  </div>
                  <div style={{ color: text3, fontSize: 13 }}>
                    {formatPeriod(e)}
                  </div>
                  {e.description && (
                    <div
                      style={{ marginTop: 6 }}
                      dangerouslySetInnerHTML={{
                        __html: e.description,
                      }}
                    />
                  )}
                </div>
              );
            })}
          </>
        )}
      </div>
    </div>
  );
}

/* ===== Sub Components ===== */
function SectionHeading({ title, icon, colorTheme, textColor, borderColor }) {
  return (
    <div
      style={{
        display: "flex",
        alignItems: "center",
        gap: 8,
        fontWeight: 600,
        fontSize: 16,
        color: textColor,
        borderBottom: `2px solid ${borderColor}`,
        paddingBottom: 4,
        marginBottom: 12,
        textTransform: "uppercase",
        background: colorTheme,
        borderRadius: 4,
        padding: "6px 12px",
      }}
    >
      {icon}
      {title}
    </div>
  );
}

const InfoRow = ({ label, value, color }) => (
  <table style={{ margin: "4px 0", fontSize: "14px", color }}>
    <tbody>
      <tr style={{ verticalAlign: "top" }}>
        <td style={{ width: 120 }}>{label}</td>
        <td style={{ width: 10 }}> : </td>
        <td>{value || "-"}</td>
      </tr>
    </tbody>
  </table>
);
